...
double StippleViewer::calDist(double x1, double y1, double x2, double y2) {
  ...
  for (int i = 1; i < solution.size(); i ++) {
    ...
    double y2 =
        static_cast<double>(stipples[solution[i]].pos.y() * m_image.height()
                                                                - stipples[solution[i]].size / 2.0f);
    double x2_ = static_cast<double>(stipples[solution[i]].pos.x() * target_height * ratio);
    double y2_ = static_cast<double>(stipples[solution[i]].pos.y() * target_height);

    if (calDist(x1_, y1_, x2_, y2_) < 0.03) {
  ...
  for (int y = 0; y < clustered.rows; y++)  {
    for (int x = 0; x < clustered.cols; x++)  {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        imgs[cluster_index].at<cv::Vec3b>(y, x)[i] = img.at<cv::Vec3b>(y, x)[i];//bgr.at<float>(0,i);//centers.at<float>(cluster_index, i);
      }
    }
  }
  if (params.saveLog)
